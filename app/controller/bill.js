...
class Billcontroller extends Controller {
  ...
  async data() {
    ...
    try {
      ...
      const total_exprense = data.reduce((arr, cur) => {
        if (cur.pay_type === 1) arr += Number(cur.amount);
        return arr;
      }, 0);
      const total_income = data.reduce((arr, cur) => {
        if (cur.pay_type === 2) arr += Number(cur.amount);
        return arr;
      }, 0);
      ...
      ctx.body = {
        ...
        msg: '请求成功',
        data: {
          total_expense: Number(total_exprense).toFixed(2),
